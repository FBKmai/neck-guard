...
| `--imgsz` | 推理分辨率，**要与手机端的推流分辨率取齐** | 960 |
| `--half` / `--no-half` | 半精度推理，GPU 上更快 | 开 |
...
**`--imgsz` 不是越大越准。** 它超过手机推来的画面尺寸后，多出来的像素全是插值放大的，模型在糊掉的放大图上反而会把耳朵或髋部认错位置。

拿一张手机端标注为 23.9° 的真实抓拍图（源图 480x640）实测，同一张图只改 imgsz：
...
1600 往上直接崩掉。所以正确做法是**先提高手机端的推流分辨率，再把 imgsz 调到同一档**：

```powershell
# 手机端设置页「推流分辨率」选 1280x960，然后
python neck_camera_monitor.py --show --imgsz 1280
```

手机端最高支持 1920x1440，但 1600 以上很吃 Wi-Fi 带宽，掉帧比分辨率低更伤判定（检测靠连续帧的时间窗投票）。建议先用 1280x960 跑一段，看状态行里的跳帧比例再决定。

### 半精度默认开启

`--half` 现在默认开。姿态关键点是像素级回归，fp16 的精度损失远小于关键点本身的抖动。18 张真实图实测，fp16 与 fp32 的角度差最大 0.17°、平均 0.06°，远小于 4° 的迟滞带，不影响任何判定。想关掉用 `--no-half`。

需要说明的是提速有限（实测约 5%），因为这个任务的瓶颈在拉流与 JPEG 解码，不在矩阵运算。
...
```
推理 9.8 fps / 推流 10.0 fps  跳帧 2%(31)  带宽 6.4 Mbps  VALID  角度 24.3  阈值 35.0  解码失败 0  重连 0
```
...
| 推理 fps | 电脑真正跑完检测的帧率 |
| 推流 fps | 手机实际推过来的帧率 |
| 跳帧 | 推来了但没进推理、被新帧顶掉的比例与累计数 |
| 带宽 | 平均占用，用来判断 Wi-Fi 是否吃紧 |

**跳帧不是网络丢包。** MJPEG 走 TCP 不会真丢包，跳掉的是客户端主动跳过的旧帧：拉流线程只保留最新一帧，推理慢于推流时中间那些就被覆盖了。这是有意设计，保证画面永远最新、延迟不累积。

怎么看这组数字：

- **跳帧低于 10%**：正常，推理跟得上。
- **跳帧 10% 到 50%**：推理慢于推流。想提高有效帧率就调低手机端推流帧率（让两边匹配），或者换小一号模型、调低 `--imgsz`。
- **跳帧超过 50%**：退出时会额外告警。此时大部分画面被浪费，判定的时间分辨率变差，建议按上面的方式调。
- **推流 fps 明显低于手机端设定值**：瓶颈在 Wi-Fi 或手机编码，调低推流分辨率或 JPEG 质量。

退出时打一份总摘要，含累计收帧、跳帧、解码失败、重连次数与总流量。
...
**画面卡顿或延迟大**：降低手机端的推流分辨率或帧率，或者调小 `--imgsz`。推流是「只保留最新帧」的，慢了会丢帧而不是堆积延迟。先看状态行里的跳帧比例判断是哪一侧慢。
